...
int main() {
  ...
  test("Test7", hs.contains(b), true); // test contains
  test("Test8", hs.remove(c), true); // test remove
  test("Test9", hs.insert(c), true); // inserts removed node
  test("Test10", hs.remove(n), true); // removes different node
  test("Test11", hs.remove(n), false); // removes a second time
