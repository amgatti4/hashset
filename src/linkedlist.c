...
int insertAtTail(LinkedList * list, void* item){
...
int insertAtHead(LinkedList * list, void* item){
...
void* removeTail(LinkedList * list){
	...
	if(list->head == NULL){
		...
		Node* prevNode = (Node*)nodeAtIndex(list, listSize(list)-2);
		free(rmNode);
		list->tail = prevNode;
		list->tail->next = NULL;
		return val;
...
bool containsNode(LinkedList* list, void* item){
	if(list == NULL){
		return 0;
	}
	// Returns NULL if list is empty
